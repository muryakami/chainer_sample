...
plt.xlabel("epoch")
plt.savefig("./pic/SAE_mnist/mean loss graph.png")
"""
model.to_cpu()
...
plt.savefig("./pic/SAE_mnist/IO_images.png")
...
def draw_digit_w1(data, n, i, length):
    print('IN : length :', length) ###
    print('math.ceil(length/15) :', math.ceil(length/15))
    size = 28
    plt.subplot(math.ceil(length/15), 15, n)
    Z = data.reshape(size,size)   # convert from vector to 28x28 matrix
    print('prevZ :', Z)
    print('prevZ.shape :', Z.shape)
    Z = Z[::-1,:]                 # flip vertical
    print('nextZ :', Z)
    print('nextZ.shape :', Z.shape)
    plt.xlim(0,size)
    plt.ylim(0,size)
    plt.pcolor(Z)
...
for i in range(len(l1_W[len(l1_W)-1])):
    print('OUT : len(l1_W[len(l1_W)-1]) :', len(l1_W[len(l1_W)-1])) ###
    draw_digit_w1(l1_W[len(l1_W)-1][i], cnt, i, len(l1_W[len(l1_W)-1]))
    cnt += 1
    
plt.savefig("./pic/SAE_mnist/w1_images.png")
...
plt.savefig("./pic/SAE_mnist/w2_T_images.png")
